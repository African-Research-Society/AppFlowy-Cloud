...
Cloud ledger unreviewed is 0 of 958 rows after batches 240–286. That is wave-1 file coverage, not a finished audit. Fixes include an HttpOnly admin cookie, redacted token logs, bound quick-note search, password reset off the command line, and collab read/write denial when the permission check is not `Ok(true)`. A leftover publish-path check in `workspace.rs` uses the same `!= Ok(true)` test and was not compiled here. Waves 2 and 3 were not run.
